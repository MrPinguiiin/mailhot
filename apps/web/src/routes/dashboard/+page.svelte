<script lang="ts">
  ...
  const setupClient = createMutation(() =>
...
{#if $sessionQuery.isPending}<div class="p-12 text-muted-foreground">
    Loading owner console...
  </div>
{:else if !$sessionQuery.data}<div class="p-12 text-muted-foreground">
...
{:else}<div class="mx-auto max-w-6xl px-6 py-12">
    ...
    <h1 class="mt-3 text-4xl font-semibold text-foreground">
      Provision client spaces
    </h1>
    ...
    <Card class="mt-8 p-6"
      ...
      >
        ...
        /><Button disabled={createClient.isPending}>Create client</Button>
      </form></Card
    >
    <div class="mt-8 grid gap-4">
      {#each dashboard.data?.clients ?? [] as client}<Card class="p-6"
          ...
          >
            <Input bind:value={domain} placeholder="example.com" /><Input
              type="password"
              bind:value={token}
              placeholder="Cloudflare API token"
            /><Button disabled={setupClient.isPending}>Verify & setup</Button>
          </form>
          {#each client.domains as item}<p
              class="mt-4 text-sm text-muted-foreground"
            >
              {item.hostname} <span class="text-primary">{item.status}</span>
              <a class="ml-2 underline" href={`/client/${client.slug}`}
                >Open page</a
              >
            </p>{/each}</Card
        >{/each}
